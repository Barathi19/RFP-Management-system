...
import { GetRFPById } from '../service/rfp';
import { GetProposalRecommendation, GetProposalsByRFPId } from '../service/proposal';
...
const RFPDetail = () => {
    ...
    const fetchProposals = async () => {
        ...
        try {
            ...
            console.log("No proposals yet or sync error");
        } finally {
        ...
        <div className="space-y-8">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="text-gray-500">Status: <span className="font-semibold text-indigo-600">{rfp.status}</span></p>
                    </div>
                </div>
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="p-4 bg-gray-50 rounded-xl">
                        ...
                        <span className="block text-lg font-semibold text-gray-900">${rfp.budgetTotal?.toLocaleString()}</span>
                    </div>
                    <div className="p-4 bg-gray-50 rounded-xl">
                        <span className="block text-sm font-medium text-gray-500">Timeline</span>
                        <span className="block text-lg font-semibold text-gray-900">{rfp.deliveryDays}</span>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                    {vendors.map(vendor => {
                        const isSent = rfp.vendorIds && rfp.vendorIds.some(v => v._id === vendor._id || v === vendor._id);
                        return (
                            <div
                                key={vendor._id}
                                onClick={() => !isSent && toggleVendor(vendor._id)}
                                className={`p-4 rounded-xl border transition-all ${isSent
                            ...
                            >
                                <div className="flex items-center space-x-3">
                                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${isSent
                                        ? 'bg-gray-300 border-gray-300'
                                        : selectedVendors.includes(vendor._id)
                                            ? 'bg-indigo-600 border-indigo-600'
                                            : 'border-gray-300'
                                        }`}>
                                        {(selectedVendors.includes(vendor._id) || isSent) && <CheckCircle className="w-3 h-3 text-white" />}
                                    </div>
                                    <div>
                                        <p className="font-medium text-gray-900">{vendor.name}</p>
                                        <p className="text-sm text-gray-500">{vendor.category} {isSent && '(Sent)'}</p>
                                    </div>
                                </div>
                            </div>
                        )
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
                <div className="flex justify-between items-center mb-6">
                    ...
                    >
                        {proposalsLoading && <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-indigo-600 mr-2"></div>}
                                ...
                                {proposals.map((proposal) => (
                                    <tr key={proposal._id}>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm font-medium text-gray-900">{proposal.vendorId?.name || "Unknown"}</div>
                                            <div className="text-sm text-gray-500">{proposal.vendorId?.email}</div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            ${proposal.parsedResponse?.totalPrice?.toLocaleString() || 0}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex space-x-3">
                        ...
                        >
                            {comparisonLoading ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-indigo-600 mr-2"></div> : 'Refresh Analysis'}
                        </button>
                    </div>
                </div>
                ...
                {comparisonLoading ? (
                    ...
                    <div className="prose max-w-none p-6 bg-indigo-50 rounded-xl">
                        <p className="whitespace-pre-wrap text-gray-800 leading-relaxed">{comparison}</p>
                    </div>
                ) : (
                    <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                        <p className="text-gray-500">No analysis generated yet. Click refresh to analyze proposals.</p>
                    </div>
                )}
            </div>
        </div >
